...
int main(int argc, char *argv[]) {
    ...
    /*
    * Encryption Client
    * args: 0 enc_client 1 plaintext 2 key 3 port
    ...
    */
    ...
    // Open the plaintext file to get input message
    ...
    // Determine length of plaintext and key files
    struct stat msgStat;
    struct stat keyStat;
    stat(argv[1], &msgStat);
    ...
    // Validate length of key compared to length of plaintext
    if( keyLen < msgLen) {
    ...
    msgCharsWritten = send(socketFD, "enc_client", 10, 0);
    ...
    // Report connetion error with the attempted port, and set exit value to 2. 
    if (strcmp(ackBuffer, "enc_server") != 0 ) {
    ...
    while(m < msgLen) {
        ...
        m += fread(msgBuffer, sizeof(char), bufLen - 1, plainTextFD);
        ...
        for(int i = 0; i < strlen(msgBuffer); i++) {
            ...
            if(( msgBuffer[i] < 65 || msgBuffer[i] > 90) && msgBuffer[i] != 32) {
                ...
                error("enc_client error: plaintext contains bad characters\n");
            }
    ...
    // Close the socket, plaintext and key files and return
    fclose(plainTextFD);
    fclose(keyFD);
    close(socketFD); 
    return 0;
