        ...
        if (CHILD_SOCKETS <= 5) {
            ...
            */
            ...
            case 0:
                ...
                ackCharsWritten = send(connectionSocket, "dec_server", 10, 0); 
                if (ackCharsWritten < 0){
                    error("ERROR writing to socket");
                }

                // Refuse and close connection to wrong client
                if (strcmp(ackBuffer, "dec_client") != 0 ) {
                    close(connectionSocket);
                    exit(2);
                }
                ...
                while(1) {
                    ...
                    /********** Decrypt message & send back data to client **********/ 

                    char charList[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ ";
                    memset(decBuffer, '\0', sizeof(decBuffer));
                    int errCh, msgCh, keyCh, decCh;

                    for(int i = 0; i < strlen(msgBuffer); i++) {

                        errCh = msgBuffer[i];
                        // Check for "bad" characters in ciphertext file
                        if(( errCh < 65 || errCh > 90) && errCh != 32) {
                            error("dec_server error: ciphertext contains bad characters");
                        }
                        ...
                        if(( errCh < 65 || errCh > 90) && errCh != 32) {
                            error("dec_server error: key contains bad characters");
                        }
                        ...
                        // put new decrypted character into buffer
                        decCh = msgCh - keyCh;
                        if( decCh < 0) {
                            decCh += 27;
                            decBuffer[i] = charList[decCh];
                        } else {
                            decBuffer[i] = charList[decCh % 27];
                        }
                    }

                    // Add newline character to end of encoded string
                    if( strlen(msgBuffer) < bufLen - 1) {
                        decBuffer[strlen(msgBuffer)] = '\n';
                    }

                    // Send decrypted message back to the client
                    do {
                        decCharsWritten = send(connectionSocket, decBuffer, strlen(decBuffer), 0); 
                        if (decCharsWritten < 0){
                            error("ERROR Server writing to socket");
                        }
                        if (decCharsWritten < strlen(decBuffer)){
                            fprintf(stderr, "CLIENT: WARNING: Not all data written to socket!\n");
                        }
                    } while (decCharsWritten < strlen(decBuffer));
                }
